...
class EssAuth extends Api {
    ...
    {                  
        ...
        $user = $this->models->load('User')->where('email', trim($parameters->email))->first();
        
        if(object_exists( $user )) {
            return $this->fault('User account matching provided email already exists', 'InvalidEmailException', 'Client');
        }
        ...
        if(isset($parameters->client_id)) { 
            $client = $this->models->load('Client', $parameters->client_id, false); 
        }
        ...
        if(object_exists($existing_client)) {
            ...
            } else if($client->id != $existing_client->id) {
                return $this->fault('Account matching provided email already exists', 'InvalidEmailException', 'Client');
            }
        }
        ...
        if(!check_date($user->dob)) {
            return $this->fault('Provided date of birth is not valid', 'InvalidDobException');
        }
