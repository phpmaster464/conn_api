...
class EssUser extends Api {
    ...
    {
    	if(!$this->simple_auth()) {
            return $this->fault($this->authentication_failed_message, 'InvalidAuthenticationException');
        }

    	if(!$this->verify_details($parameters)) {
            return $this->fault($this->parameters_failed_message, 'InvalidParameterException');
        }

        $hash = $parameters->verify_details->hash;
        
        if($this->has_expired($hash)) {
            return $this->fault($this->expired_message, 'TokenExpiredException');
        }
    ...
    {    	    
        ...
        if(object_exists($user))
        {
            ...
            if(isset($parameters->client_details)) {
                ...
                $parameters->client_details->title = $user->title;
                ...
                #CRN
                if(isset($parameters->client_details->crn)) {
                    $parameters->client_details->crn = clean_value($parameters->client_details->crn);
                }

                #DRIVER LICENSE
                if(isset($parameters->client_details->license->number)) {
                    $parameters->client_details->driver_license = clean_value($parameters->client_details->license->number);
                }

                if(isset($parameters->client_details->license->state_code)) {
                    $parameters->client_details->driver_license_state_code = clean_value( $parameters->client_details->license->state_code );
                }

                #MEDICARE
                if(isset($parameters->client_details->medicare->number)) {
                    $parameters->client_details->medicare_card = clean_value($parameters->client_details->medicare->number);
                }

                if(isset($parameters->client_details->medicare->expiry_date_month) && isset($parameters->client_details->medicare->expiry_date_year) ) {
                    $parameters->client_details->medicare_card_expiry = clean_value( $parameters->client_details->medicare->expiry_date_month ).'/'.clean_value( $parameters->client_details->medicare->expiry_date_year );
                }

                if(isset($parameters->client_details->medicare->reference_number)) {
                    $parameters->client_details->medicare_card_reference = clean_value($parameters->client_details->medicare->reference_number);
                }

                if(isset($parameters->client_details->medicare->middle_name)) {
                    $parameters->client_details->medicare_card_middle_name = clean_value($parameters->client_details->medicare->middle_name);
                }

                if(isset($parameters->client_details->medicare->card_colour)) {
                    $parameters->client_details->medicare_card_colour = clean_value($parameters->client_details->medicare->card_colour);
                }

                #PASSPORT
                if(isset($parameters->client_details->passport->number)) {

                    $parameters->client_details->passport = clean_value($parameters->client_details->passport->number);

                    $country = false;

                    if(isset($parameters->client_details->passport->country))
                    {
                        $country_name = trim( clean_value( $parameters->client_details->passport->country ) );   
                        ...
                        $country_code = trim( clean_value( $parameters->client_details->passport->country_code ) ); 
                    ...
                    if(!object_exists($country)) {
                        return $this->fault($this->invalid_country_message, 'InvalidCountryException');
                    }
                    
                    $parameters->client_details->passport_country_code = $country->prefix;
                }                
            ...
            if(is_object($client_income)) {
                $guzzle->request('POST', 'clients/'.$client->id.'/income/add', ['form_params'=>$client_income ] );
            }
            
            if(is_object($client_deductions)) {
                $guzzle->request('POST', 'clients/'.$client->id.'/deductions/add', ['form_params'=>$client_deductions ] );
            }

            if(isset($parameters->client_details->different_delivery_address) && $parameters->client_details->different_delivery_address) {
                $guzzle->request('POST', 'clients/'.$client->id.'/delivery-address/edit/'.$client->id, ['form_params'=>$parameters->client_details ] );
            } else {
                $guzzle->request('GET', 'clients/'.$client->id.'/delivery-address/process-remove' );
            }
    ...
    public function add_budget_type($object, $client, $type, $guzzle )
    {
        if(is_array($object)) {
            ...
            foreach($object as $income) { 
                $this->add_budget_type($income, $client, $type, $guzzle ); 
            }
        
        } else if(is_object($object)) {

            if(isset($object->id) && $object->id != '' && $object->id != 0) { 

                $c_budget = $this->models->load($type, $object->id); 

                if(!object_exists($c_budget)) {
                    $c_budget = $this->models->load($type);
                }
